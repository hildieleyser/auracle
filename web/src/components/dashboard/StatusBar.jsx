 ...
 *
 *   live    bridge connected, last sample fresh
 *   demo    bridge offline, dashboard is showing synthesized values
 *   stale   bridge connected but last sample > 15s old
 *   waiting bridge connected, no samples yet
 */
export default function StatusBar({ status }) {
  ...
  if (status.source === 'connecting') {
    kind = 'connecting';
    label = 'Connecting';
    detail = `Looking for bridge at ${status.bridgeUrl}`;
  } else if (status.source === 'demo') {
    kind = 'demo';
    label = 'Demo';
    detail = 'Bridge offline — values are simulated';
  } else if (ageSec == null) {
    kind = 'waiting';
    label = 'Waiting';
    detail = 'Bridge connected — no samples yet';
  } else if (ageSec > 15) {
    kind = 'stale';
    label = 'Stale';
  ...
  return (
    <section className="flex items-center justify-between rounded-2xl border border-hairline bg-parchment/70 px-5 py-4">
